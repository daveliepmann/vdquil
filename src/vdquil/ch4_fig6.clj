;; Ben Fry's Visualizing Data, Chapter 4 (Time Series), figure 6:
;; Time series with data set and axes labeled, vertical grid and tick marks.
  ...
  (let [plotx1 120
    ...
    (doseq [row @(state :ymtc)]
      (let [year (first row)
            ...
            tea (nth row 2)
            coffee (nth row 3)]
        (point (map-range year year-min year-max plotx1 plotx2) (map-range (nth row @current-column) data-min data-max ploty2 ploty1))))))
