...
(defn draw []
  ...
  (doseq [location @(state :locations)]    
    (let [abbrev (first location)
          ...
          radius 15
          percent (norm randomValue (apply min (map #(second %) @(state :random))) (apply max (map #(second %) @(state :random))))
          lowColor (hex-to-rgb "#296F34") ;; ruddy green
          highColor (hex-to-rgb "#61E2F0") ;; light blue
          ;;;; to produce figure 3-3 (red to blue) swap the following and previous two lines:
          ;;lowColor (hex-to-rgb "#FF4422") ;; red
          ;;highColor (hex-to-rgb "#4422CC") ;; blue
          ]
      (smooth)
      (no-stroke)
      (fill (lerp-color (color (first lowColor) (second lowColor) (nth lowColor 2)) (color (first highColor) (second highColor) (nth highColor 2)) percent))
