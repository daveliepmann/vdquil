;; Ben Fry's Visualizing Data, Chapter 4 (Time Series), figure 11:
;; Curve vertices
;; Converted from Processing to Quil as an exercise by Dave Liepmann

(ns vdquil.chapter4.figure11
  (:use [quil.core]
        [vdquil.util]
...
(defn draw-data-point [row]
  ...
  (let [[year milk tea coffee] row]
    (curve-vertex (map-range year year-min year-max plotx1 plotx2)
                  (map-range (nth row (get-current-column))
                             data-min data-max ploty2 ploty1))))
