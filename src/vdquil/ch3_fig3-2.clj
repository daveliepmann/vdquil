  ...
  (doseq [location @(state :locations)]    
    (let [abbrev (first location)
          ...
          radius (if (>= randomValue 0)
                   (map-range randomValue 0 (apply max (map #(second %) @(state :random))) 2 40)
                   (map-range randomValue 0 (apply min (map #(second %) @(state :random))) 2 40))
          ;; to produce figure 3-1, use "radius 9" instead of the preceding 3 lines
          ]
      (smooth)
      (no-stroke)
      (fill 192 0 0)  
